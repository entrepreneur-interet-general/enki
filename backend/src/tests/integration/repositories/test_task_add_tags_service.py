...
from domain.messages.ports.task_repository import AbstractTaskRepository
from uuid import uuid4


def test_add_task_and_add_tag(task_repo: AbstractTaskRepository, tag_repo: AbstractTagRepository):
    if isinstance(tag_repo, PgRepositoryMixin) and isinstance(task_repo, PgRepositoryMixin):
        task_uuid = str(uuid4())
        tag_uuid = str(uuid4())
