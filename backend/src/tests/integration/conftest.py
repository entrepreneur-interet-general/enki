...
def tag_pg_repo(sqlite_engine: Engine) -> AbstractTagRepository:
    ...
    tag_repository = PgTagRepository(sessionmaker(bind=sqlite_engine)())
...
def _tag_repo(request, tag_in_memory_repo, tag_pg_repo) -> AbstractTagRepository:
...
@pytest.fixture(scope="function", params=["in_memory"])  # , "sqlite"
def _task_repo(request, task_in_memory_repo, task_pg_repo) -> AbstractTaskRepository:
    if request.param == "sqlite":
        repo = task_pg_repo
        repo._tasks = []
    else:  # request.param == "in_memory"
